...
namespace SpotifyApi
{
    public class Spotify
    {
        ...
        public Spotify(string OAuth, string Host = "127.0.0.1")
        {
            _oauth = OAuth;
            _host = Host;
        ...
        public static string GetOAuth()
        {
            var wc = new WebClient();
            wc.Headers.Add("User-Agent: SpotifyAPI");
            var raw = wc.DownloadString("https://open.spotify.com/token"); // Retrieve the raw string
            try
            {
                var token = raw.Substring(6, 68); // Let's take the 68-character OAuth key.
                return token;
            }
            catch (Exception e)
            {
        ...
        private string recv(string request, bool oauth, bool cfid, int wait)
        {
            ...
            catch (Exception z)
            {
                ...
                if (System.Diagnostics.Process.GetProcessesByName("SpotifyWebHelper").Length < 1)
                {
                    try
                    {
